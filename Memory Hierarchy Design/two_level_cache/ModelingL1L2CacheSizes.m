...
batchGranularity = 2500
count = 0;

for L1Index = 1:length(CacheData)
    for L2Index = 1:length(CacheData)
        
        L1Bits = CacheData(L1Index, 4);
        L2Bits = CacheData(L2Index, 4);
        L1L2TotalBits = L1Bits + L2Bits;
        
        L1BlockSize = CacheData(L1Index, 1);
        L2BlockSize = CacheData(L2Index, 1);
        
        if L1L2TotalBits <= 1500000 && L2BlockSize >= L1BlockSize
            ...
            if mod(count, batchGranularity) == 0
                ...
                filename = ['./two_level_cachesim_sh_files/simulations_all_', int2str(count/batchGranularity), '.sh']
                f = fopen(filename, 'w');
            ...
            CacheData(L1Index, :);
            CacheData(L2Index, :);
            fprintf(f, '\techo -n "$t %d %d %d %d %d %d - %d - " > ./results/cachesim_result_%d_$t.txt\n', CacheData(L1Index, 1:3), CacheData(L2Index, 1:3), L1L2TotalBits, count);
            fprintf(f, '\t./cachesim $t %d %d %d %d %d %d >> ./results/cachesim_result_%d_$t.txt &\n', CacheData(L1Index, 1:3), CacheData(L2Index, 1:3), count);
            % fprintf(f, '%d \n', L1L2TotalBits);
            % fprintf(f, '\n');
        end
